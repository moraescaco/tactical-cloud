...
class PgConnCompat:
    ...
    def _translate_sql(self, sql):
        ...
        # PostgreSQL converte aliases não cotados para minúsculas. O app original foi
        # feito para SQLite e usa aliases como Nome, Data, Receita, Valor etc. Aqui
        # preservamos os aliases e também corrigimos ORDER BY/GROUP BY que usam esses
        # nomes sem aspas. Sem isso o Postgres procura colunas reais em minúsculas
        # e gera erros como: column "receita" does not exist.
        aliases = []
        ...
        sql = re.sub(
            r'AS\s+([A-Za-z_À-ÿ][A-Za-z0-9_À-ÿ]*)',
            quote_alias,
            sql,
            flags=re.IGNORECASE,
        )
