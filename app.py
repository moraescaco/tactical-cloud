...
def select_section(label, options, key, default=None, show_label=False):
...
def init_db():
    ...
    with get_conn() as conn:
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_commands_event_id ON commands(event_id)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_commands_operator_id ON commands(operator_id)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_commands_opened_at ON commands(opened_at)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_sales_command_id ON sales(command_id)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_sales_event_id ON sales(event_id)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_sales_product_id ON sales(product_id)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_sales_sale_date ON sales(sale_date)")
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_stock_movements_product_id ON stock_movements(product_id)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_stock_movements_movement_date ON stock_movements(movement_date)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_cash_sessions_status ON cash_sessions(status)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_cash_movements_session_id ON cash_movements(session_id)")
...
def _cache_version():
    return int(st.session_state.get("_db_cache_version", 0))


def _bump_cache_version():
    st.session_state["_db_cache_version"] = _cache_version() + 1


def _profile_query_result(start_time, sql, params_tuple, df):
    if start_time is not None:
        elapsed = time.perf_counter() - start_time
        if elapsed >= SLOW_QUERY_SECONDS:
            sql_preview = re.sub(r"\s+", " ", str(sql)).strip()[:600]
            print(f"Tactical SQL slow query: {elapsed:.3f}s | rows={len(df)} | sql={sql_preview} | params={params_tuple}")
    return df


@st.cache_data(ttl=45, show_spinner=False)
def _query_df_cached(sql, params_tuple, use_postgres, cache_version):
    params_tuple = tuple(params_tuple or ())
    start_time = time.perf_counter() if SQL_PROFILE else None
    with get_conn() as conn:
        if use_postgres:
            translated = conn._translate_sql(sql)
            if translated is None:
                return _profile_query_result(start_time, sql, params_tuple, pd.DataFrame())
            cur = conn._conn.cursor()
            try:
                cur.execute(translated, params_tuple)
                rows = cur.fetchall()
                if rows and isinstance(rows[0], dict):
                    return _profile_query_result(start_time, sql, params_tuple, pd.DataFrame([dict(r) for r in rows]))
                columns = [desc[0] for desc in (cur.description or [])]
                return _profile_query_result(start_time, sql, params_tuple, pd.DataFrame(rows, columns=columns))
                ...
                return _profile_query_result(start_time, sql, params_tuple, pd.DataFrame())
        return _profile_query_result(start_time, sql, params_tuple, pd.read_sql_query(sql, conn, params=params_tuple))


def query_df(sql, params=()):
    return _query_df_cached(str(sql), tuple(params or ()), USE_POSTGRES, _cache_version()).copy()


def query_scalar(sql, params=(), column=None, default=0, cast=None):
...
def get_event_financials(event_id):
    ...
    summary = query_df(
        f"""
        SELECT
            COALESCE((SELECT SUM(revenue) FROM sales WHERE event_id = ?), 0) AS receita_produtos,
            COALESCE((SELECT SUM(cogs) FROM sales WHERE event_id = ?), 0) AS custo_produtos,
            COALESCE((SELECT SUM(discount_amount) FROM commands WHERE event_id = ? AND status = 'Fechada'), 0) AS descontos_comandas,
            COALESCE((SELECT SUM(amount) FROM expenses WHERE event_id = ?), 0) AS despesas,
            COALESCE((SELECT SUM(CASE WHEN {tipo_expr} = 'Fixa' THEN amount ELSE 0 END) FROM expenses WHERE event_id = ?), 0) AS despesas_fixas,
            COALESCE((SELECT SUM(CASE WHEN {tipo_expr} = 'Variável' THEN amount ELSE 0 END) FROM expenses WHERE event_id = ?), 0) AS despesas_variaveis
        """,
        (event_id, event_id, event_id, event_id, event_id, event_id),
    ).iloc[0]
    return {
        "descontos_comandas": float(summary["descontos_comandas"]),
        "receita_produtos": float(summary["receita_produtos"]),
        "custo_produtos": float(summary["custo_produtos"]),
        "despesas": float(summary["despesas"]),
        "despesas_fixas": float(summary["despesas_fixas"]),
        "despesas_variaveis": float(summary["despesas_variaveis"]),
    }
...
def event_linked_counts(event_id):
    ...
    row = query_df(
        """
        SELECT
            (SELECT COUNT(*) FROM commands WHERE event_id = ?) AS comandas,
            (SELECT COUNT(*) FROM sales WHERE event_id = ?) AS vendas,
            (SELECT COUNT(*) FROM expenses WHERE event_id = ?) AS despesas,
            (SELECT COUNT(*) FROM stock_movements WHERE event_id = ?) AS movimentacoes
        """,
    ...
    return {
        ...
        "despesas": int(row["despesas"]),
        "movimentações": int(row["movimentacoes"]),
    }
...
def count_open_events():
    return query_scalar(
        "SELECT COUNT(*) AS total FROM events WHERE COALESCE(status, 'Aberto') = 'Aberto'",
        column="total",
        default=0,
        cast=int,
    )
...
def count_open_commands_for_event(event_id):
    return query_scalar(
        "SELECT COUNT(*) AS total FROM commands WHERE event_id = ? AND status = 'Aberta'",
        (event_id,),
        column="total",
        default=0,
        cast=int,
...
def finalize_event(event_id):
    open_commands = count_open_commands_for_event(event_id)
    if open_commands > 0:
...
def date_filter_sql(column, start_date, end_date):
    end_exclusive = pd.to_datetime(end_date).date() + timedelta(days=1)
    return f"{column} >= ? AND {column} < ?", (str(start_date), str(end_exclusive))


def kpis(start_date, end_date):
    period_sql, params = date_filter_sql("event_date", start_date, end_date)
    event_totals = query_df(
        f"""
        SELECT
            COALESCE(SUM(entry_revenue), 0) AS receita_entradas,
            COALESCE(SUM(players), 0) AS jogadores
        FROM events
        WHERE {period_sql}
        """,
        params,
    )
    events = float(event_totals["receita_entradas"].iloc[0])
    players = float(event_totals["jogadores"].iloc[0])
    ...
    period_sql, params = date_filter_sql("COALESCE(closed_at, opened_at)", start_date, end_date)
    discounts = query_df(
        """
...
def page_cash():
    ...
    cash_section = select_section("Área do caixa", ["Caixa atual", "Histórico de caixas"], "cash_section", default="Caixa atual")

    if cash_section == "Caixa atual":
    ...
    elif cash_section == "Histórico de caixas":
        sessions = query_df(
            """
...
def page_dashboard():
    ...
    with status_cols[0]:
        if open_cash_session is not None:
            st.success(f"Caixa aberto • Saldo esperado: {brl(caixa_esperado)}")
        else:
    ...
    status_counts = query_df(
        """
        SELECT
            (SELECT COUNT(*) FROM commands WHERE status = 'Aberta') AS comandas_abertas,
            (SELECT COUNT(*) FROM products WHERE active = 1 AND stock_qty <= min_stock) AS estoque_baixo
        """
    )
    open_cmd_count = int(status_counts.iloc[0]["comandas_abertas"]) if not status_counts.empty else 0
    low_stock_count = int(status_counts.iloc[0]["estoque_baixo"]) if not status_counts.empty else 0
    ...
    with left_col:
        ...
        period_sql, period_params = date_filter_sql("s.sale_date", start_date, end_date)
        vendas_cat = query_df(
            f"""
    ...
    with right_col:
        ...
        period_sql, period_params = date_filter_sql("c.opened_at", start_date, end_date)
        top_ops = query_df(
            f"""
    ...
    with event_col:
        ...
        period_sql, period_params = date_filter_sql("e.event_date", start_date, end_date)
    ...
    with st.expander("Ver comandas do período"):
        carregar_comandas = st.checkbox("Carregar comandas do período", value=False, key="dash_load_period_commands")
        if carregar_comandas:
            period_sql, period_params = date_filter_sql("c.opened_at", start_date, end_date)
            comandas = query_df(
                f"""
                SELECT c.number AS Comanda,
            ...
            st.caption("Marque a opção acima para buscar a lista completa no banco.")
...
def page_commands():
    ...
    command_section = select_section(
        "Área de comandas",
        ["1. Abrir comanda", "2. Operar comanda", "3. Histórico"],
        "commands_section",
        default="1. Abrir comanda",
    )

    if command_section == "1. Abrir comanda":
    ...
    elif command_section == "2. Operar comanda":
    ...
    elif command_section == "3. Histórico":
...
def page_events_operation():
    ...
    selected_tab = select_section("Etapa", tab_options, "events_tab_choice", default=current_tab)
    ...
    if selected_tab == "1. Criar jogo":
        ...
        comandos_abertas_qtd = count_open_commands_for_event(event_id)
...
def page_stock():
    ...
    stock_section = select_section("Área de estoque", ["Entrada de Estoque", "Movimentações"], "stock_section", default="Entrada de Estoque")
    if stock_section == "Entrada de Estoque":
        page_stock_entries()
    elif stock_section == "Movimentações":
...
def page_reports():
    ...
    start_date, end_date = render_period_filter()
    report_section = select_section(
        "Área de relatórios",
        ["Vendas", "Despesas", "Compras de estoque", "Estoque atual", "Movimentações de estoque"],
        "reports_section",
        default="Vendas",
    )

    if report_section == "Vendas":
        period_sales_sql, period_sales_params = date_filter_sql("s.sale_date", start_date, end_date)
        vendas = query_df(
            f"""
    ...
    elif report_section == "Compras de estoque":
        period_expense_sql, period_expense_params = date_filter_sql("expense_date", start_date, end_date)
    ...
    elif report_section == "Estoque atual":
        estoque = query_df(
            """
    ...
    elif report_section == "Movimentações de estoque":
        period_movements_sql, period_movements_params = date_filter_sql("sm.movement_date", start_date, end_date)
        movimentos = query_df(
            f"""
            ...
            JOIN products p ON p.id = sm.product_id
            LEFT JOIN events e ON e.id = sm.event_id
            WHERE {period_movements_sql}
            ORDER BY sm.movement_date DESC, sm.id DESC
            """,
            period_movements_params,
        )
...
def page_users():
    ...
    users_section = select_section(
        "Área de usuários",
        ["Cadastrar usuário", "Usuários cadastrados", "Logs do sistema"],
        "users_section",
        default="Cadastrar usuário",
    )

    if users_section == "Cadastrar usuário":
    ...
    elif users_section == "Usuários cadastrados":
        users = query_df(
            """
    ...
    elif users_section == "Logs do sistema":
        logs = query_df(
            """
...
def page_logs():
    ...
    period_sql, period_params = date_filter_sql("l.created_at", start_date, end_date)
    where = [period_sql]
