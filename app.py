...
def query_df(sql, params=()):
    return _query_df_cached(str(sql), tuple(params or ()), True, _cache_version()).copy()
...
def clear_all_data():
    """Limpa todas as informações do banco PostgreSQL ativo."""
    with get_conn() as conn:
...
def page_settings():
    ...
    st.write("Banco de dados cloud:", "PostgreSQL")
    st.caption("Conexão configurada por `DATABASE_URL` no ambiente do Streamlit Cloud.")
...
Os dados ficam armazenados no banco PostgreSQL configurado para o ambiente cloud. O responsável deve proteger acessos, segredos, backups e permissões de usuário.
...
"""
    ...
    st.info("Esta versão é Postgres-only. Backup e restauração devem ser feitos pelo provedor do banco, como Supabase ou Neon.")
    ...
    if st.button("Apagar todas as informações", disabled=not can_delete_system_data()):
        if confirm == "APAGAR":
            clear_all_data()
            bootstrap_database()
            st.success("Todas as informações foram apagadas do PostgreSQL ativo.")
            st.rerun()
        else:
