...
class PgConnCompat:
    ...
    def _translate_sql(self, sql):
        ...
        sql = sql.replace('strftime("%d/%m/%Y", e.event_date)', "to_char(e.event_date::date, 'DD/MM/YYYY')")
        sql = sql.replace("?", "%s")
        # PostgreSQL converte aliases não cotados para minúsculas. Como o sistema usa aliases
        # como Nome, Data, Operador_ID etc., preservamos o nome cotando automaticamente.
        sql = re.sub(
...
def page_events_operation():
    ...
    if selected_tab == "1. Criar jogo":
        ...
        produtos_vendidos_df = query_df(
            ...
            ORDER BY p.category, Quantidade_vendida DESC, p.name
            """,
            (event_id,),
        )
